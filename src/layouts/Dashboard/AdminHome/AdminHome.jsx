...
import axiosSecure from "../hooks/useAxiosSecure"; // Assuming axiosSecure is properly set up
...
const AdminHome = () => {
  ...
  const handleApprove = async (id) => {
    try {
      const response = await axiosSecure.patch(`/admin/withdrawals/${id}`);
            ...
            <tbody>
              {withdrawals.map((withdrawal, index) => (
                <tr key={withdrawal._id}>
                  ...
                  <td className="p-3">{withdrawal.email}</td>
